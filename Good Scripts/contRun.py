from selenium import webdriver
from selenium.webdriver.common.by import By
import pandas as pd
from datetime import datetime, timedelta

def scrape_data(start_date, end_date, dfs = None):
    if dfs is None:
        dfs = []
    
    max_page = 9
    driver = webdriver.Safari()
    driver.implicitly_wait(10)
    page = 1
    last_scraped_date = end_date

    while page <= max_page:
        url = f"http://openinsider.com/screener?s=&o=&pl=&ph=&ll=&lh=&fd=-1&fdr={start_date}+-+{end_date}&td=0&tdr=&fdlyl=&fdlyh=&daysago=&xp=1&vl=&vh=&ocl=&och=&sic1=-1&sicl=100&sich=9999&grp=0&nfl=&nfh=&nil=&nih=&nol=&noh=&v2l=&v2h=&oc2l=&oc2h=&sortcol=0&cnt=1000&page={page}"
        try:
            driver.get(url)
            print(f"Page {page}:")
            print(f"URL: {url}")
            table = driver.find_element(By.CLASS_NAME, 'tinytable')
            print(f"Found Table. Beginning scrape of page {page}.")
            rows = table.find_elements(By.TAG_NAME, 'tr')
            data = []
            for row in rows:
                cells = row.find_elements(By.TAG_NAME, 'td')
                if not cells:
                    cells = row.find_elements(By.TAG_NAME, 'th')
                data.append([cell.text for cell in cells])

            print(f"Finished scraping page {page}. Saving Data")
            df = pd.DataFrame(data[1:], columns=data[0])
            df.columns = df.columns.str.replace('\xa0', ' ').str.strip()
            df['Filing Date'] = pd.to_datetime(df['Filing Date'])  
            print(f"Data from page {page}: \n{df.head(-1)}")
            dfs.append(df)

            if not df.empty:
                last_scraped_date_str = df['Filing Date'].iloc[-1]
                last_scraped_date = last_scraped_date_str.date()

            page += 1

        except Exception as e:
            print(f"Scraped All Data (or encountered an error). \nCorresponding Error: \n{e}")
            break
    
    driver.quit()

    if page > max_page and start_date < last_scraped_date:
        new_end_date = last_scraped_date - timedelta(days=1)
        print(f"Recursive Call Made. Now Scraping Between {start_date} and {new_end_date}")
        return scrape_data(start_date, new_end_date, dfs)

    else:
        return dfs

def concat_and_finish(dfs):
    final_df = pd.concat(dfs, ignore_index=True).drop_duplicates()
    print(f"Columns: {final_df.columns}")
    print(f"Result: {final_df}")
    final_df.to_csv("2016Scrape.csv", index=False)

def main():
    min_date = datetime(2016, 1, 1).date()
    max_date = datetime(2016, 12, 31).date()

    dfs = scrape_data(min_date, max_date)
    if dfs:
        print("Scraped Data.")
        concat_and_finish(dfs)
    else:
        print("No data was scraped.")

if __name__ == "__main__":
    main()
