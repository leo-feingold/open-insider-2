...
def scrape_data(start_date, end_date):
    ...
    while True:
        ...
        try:
            ...
            print(f"Located final page of data between {start_date} and {end_date}: {page}\nCorresponding Error Message: \n{e}")
            break
...
def concat_and_finish(dfs):
    ...
    final_df.to_csv("december2023TestScrape.csv", index=False)

def main():
    min_date = datetime(2023, 10, 1).strftime("%m/%d/%Y")
    max_date = datetime(2023, 12, 31).strftime("%m/%d/%Y")
